			...
			<CardContent className="p-0 sm:p-6">
				{loading ? (
					...
					<div className="text-center py-12 px-4">
						...
						<p className="text-slate-500 font-medium">Tidak ada data pencapaian</p>
					...
					<>
							...
							<Table>
								...
								<TableBody>
									{filteredData.map((item) => (
										<TableRow key={`${item.id}-${item.kode_pencapaian}`} className="hover:bg-slate-50/50">
						...
						<div className="md:hidden divide-y divide-slate-100 border-t border-slate-100">
							{filteredData.map((item) => (
								<div key={`${item.id}-${item.kode_pencapaian}`} className="p-4 space-y-3 active:bg-slate-50 transition-colors">
									...
									<div className="bg-slate-50/50 border border-slate-100 rounded-lg p-2 flex items-center justify-between">
										<span className="text-xs font-semibold text-emerald-600">{item.nama_pencapaian}</span>
